-- poblar la base con datos representativos y consistentes.
...
('Hotel Centro Historico', 'Avenida Central #45', 'San Salvador', '2222-0202', 'contacto@centrohistorico.com', 4);
...
-- Habitaciones: 60 habitaciones distribuidas en 2 hoteles.
INSERT INTO habitacion (id_hotel, id_tipo_habitacion, numero, piso, estado)
SELECT
    CASE WHEN gs <= 30 THEN 1 ELSE 2 END AS id_hotel,
    CASE
    ...
    ((CASE WHEN gs <= 30 THEN 1 ELSE 2 END) * 100 + gs)::TEXT AS numero,
    ((gs - 1) % 5) + 1 AS piso,
    'DISPONIBLE'
FROM generate_series(1, 60) AS gs;

-- Huespedes: 120 registros para evitar datos insuficientes.
...
SELECT
    CASE WHEN gs <= 6 THEN 1 ELSE 2 END,
    'Empleado' || gs,
    'Hotel' || gs,
    CASE
...
-- Estancias para reservas finalizadas o con check-in.
...
-- Consumos de servicios: 180 consumos asociados a estancias.
...
-- Facturas para las estancias finalizadas.
INSERT INTO factura (
    id_estancia,
    numero_factura,
    subtotal_habitacion,
